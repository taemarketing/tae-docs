...
### Status Update — August 16, 2026: the "small builds" from the Brand & Web Presence bucket

Direct instruction: tackle the small builds first (data that already exists, just needed one more tool), hold the platform-integration layer (Google Business/Yelp/Facebook/Instagram — needed by 5 of 11 tasks in that bucket) until after Google Cloud setup lands.

- ✅ **`get_traffic_anomaly` shipped — closes Daily Traffic Anomaly Check.** Real Vercel Web Analytics data (same source `get_site_analytics` uses): pulls 14 days, compares the latest day's pageviews against the average of the 7 days before it, flags spike/drop past a configurable threshold (default 50%). Verified live against F.R.I.: correctly flagged a real -100% drop (an early-morning check on a low-traffic site showing 0 pageviews so far that day — an honest, plausible result, not a bug).
- ✅ **`get_core_web_vitals` shipped — closes part of Site Health & SEO Hygiene Check.** Real Core Web Vitals (LCP, CLS, INP) and a performance score via Google's PageSpeed Insights API, works off `core.companies.domain` directly — no Vercel mapping needed, unlike most of this bucket's other tools. **A real finding from the live test, not a theoretical caveat:** the unauthenticated endpoint returned a 429 immediately — not a per-second rate limit as first assumed, but `"Quota exceeded... Queries per day"`, a small daily quota **shared across every anonymous caller of that endpoint worldwide**, meaning it can fail for reasons that have nothing to do with TAE's own usage. Corrected the tool's description and error message to say this precisely instead of the wrong guess. `GOOGLE_PAGESPEED_API_KEY` (planned for Monday, a real Google Cloud project's own key) gives a dedicated quota — not a nice-to-have, the actual fix for a real, already-observed failure mode.
- ✅ **Two Ready-badge corrections, both real oversights caught while reviewing this bucket, not new capability.** `Site Uptime Pulse`'s description still said "Blocked — needs an actual uptime pinger; none exists yet," despite `get_site_uptime` having existed and worked for a long time — a stale description, not a stale feature. Fixed and added to `READY_CADENCE_TASKS` alongside `Daily Traffic Anomaly Check` (now genuinely Ready, not just "buildable"). `Site Health & SEO Hygiene Check` deliberately stays out of the Ready set — uptime/broken-links/sitemap/CWV are real now, but the meta-description/alt-text/schema-markup check and the GEO check itself are both still genuinely unbuilt, so "Partially buildable" remains the honest label, not Ready.
- **What's still open in this bucket:** the 5 tasks needing Google Business Profile/Yelp/Facebook/Instagram connections (Reputation & Review Sweep, Directory & Profile Consistency Check, New Review Alert, Social Mentions & DM Check, Holiday Hours & Closure Sweep) — deliberately held for after Monday's Google Cloud setup, since they share one root blocker rather than being 5 separate builds. Real-Time Site Health Alerting and Live Status Propagation both need continuous/event-driven infrastructure that a one-off tool call can't provide, and Real-Time alerting specifically runs into the standing no-scheduled-jobs rule — worth a real decision (external monitoring service vs. waiting for beta) before building, not just "build it later."

### Status Update — later August 16, 2026: Real-Time Site Health Alerting decided; 4 more Marketing Performance tasks shipped

**The Real-Time Site Health Alerting decision, made:** an external monitoring service does the actual continuous polling on its own infrastructure — TAE never runs its own cron, so the standing no-scheduled-jobs rule doesn't apply (no Anthropic API involved, no TAE-triggered automation, the "schedule" lives on a vendor's servers). Real vendor research, not assumption — two candidates turned out to have a live-verified catch: **UptimeRobot's free tier explicitly bans commercial use since December 2024** (business/client work risks account suspension), and **Better Stack's free tier is literally labeled "Free for personal projects"** on their actual pricing page — both would have been wrong recommendations if taken from secondhand summaries instead of the real page. **StatusCake was chosen instead**: free tier (3 monitors, 5-minute checks, email alerts, "free forever"), framed as "best for individuals, **small sites**" rather than personal-only, with a $24.49/month paid tier to grow into. `TAE_Google_Cloud_Setup_Instructions.md` (on the Desktop) updated with a StatusCake account-creation step and a Search Console confirmation step, alongside the existing PageSpeed/Site-Verification-API instructions — all three now live in one Monday setup doc.
- ✅ **`get_top_performer`, `get_business_pulse`, `get_underperforming_pages` shipped — 3 tools closing 4 cadence tasks.** All three wrap the same real Vercel Web Analytics data `get_site_analytics`/`get_traffic_anomaly` already use, pre-computed for a specific question:
  - `get_top_performer` — highest-traffic page over a window. Closes **Top Performer of the Week**.
  - `get_business_pulse` — traffic vs. the prior equivalent window, defaulting to weekly but accepting a custom date range. One tool serves two cadence tasks: **Weekly Business Pulse Report** (default 7-day window) and **Post-Holiday Performance Recap** (a holiday-window call to the same tool) — not two separate builds. **Honest gap stated by the tool itself, not glossed over:** it reports traffic only. "Leads" and "conversions" from the original task descriptions aren't tracked anywhere in TAE's tools for a client's *own* business — Discovery tracks prospects for TAE itself, a fundamentally different thing that would have been dishonest to reuse here just because it was the nearest available data.
  - `get_underperforming_pages` — flags pages below a threshold percent of the top page's traffic (default 10%). Closes the traffic-identification half of **Underperforming Page/Content Audit**; drafting actual improvement recommendations from the flagged list still needs a person or a future AI-assisted pass, noted explicitly in the task's own description rather than claimed as done.
- ✅ **All three verified live against F.R.I.'s real single-page site, including a real edge case handled gracefully.** F.R.I. has no distinct routes (it's one static page), so Vercel's route breakdown returns a single entry with `route: ""` — all three tools handled this correctly rather than erroring: `get_top_performer` returns it as the (only) top page, `get_underperforming_pages` correctly reports zero underperforming pages since there's nothing to compare it against. `get_business_pulse` showed a real +100% traffic change because the prior week genuinely had zero traffic (matches the site's actual launch date).
- ✅ **8 tasks now Ready across the two Brand & Web Presence / Marketing Performance passes today:** Site Uptime Pulse, Daily Traffic Anomaly Check, Top Performer of the Week, Weekly Business Pulse Report, Post-Holiday Performance Recap, Underperforming Page/Content Audit, plus the two corrected earlier the same day.
...
- [ ] **Connect Google Search Console.** Today the only traffic data source is Vercel Web Analytics — there is no ranking data at all. Without this, none of the SEO work below can be verified as actually working.
  ...
  **Decided 2026-08-13: automated self-verification, not a client-facing OAuth popup.** A "connect your Google account" button was considered and rejected — even with client consent, if the property isn't already verified under that Google account, nothing gets created; OAuth grants *delegation*, not *verification*. The real fix: Google's Site Verification API supports verifying ownership via an HTML meta tag. TAE requests a verification token, writes it into the site's `<head>` via the `head_tag` field mechanism (already built, same one used for meta description/canonical/OG), and confirms verification — all server-side, zero client interaction, same process for every future client regardless of what's already set up on their end. Once verified, the property exists under **TAE's own** Google account permanently.

  **What Monday's session needs to produce, to unblock building this:** a Google Cloud project under TAE's control with the Site Verification API enabled, plus credentials for it (a one-time setup, not per-client — set directly by whoever holds the Google account, same handling discipline as `VERCEL_API_TOKEN`/`GITHUB_API_TOKEN`, never typed by the assistant). Once that exists, a new `verify_site_ownership` ContextLayer tool is straightforward to build against real credentials.

  **The `access_requests` checklist system (built 2026-08-13, migrations 319–320) stays as the fallback path** — for any future case automated verification can't handle (a client-owned/hosted site outside TAE's GitHub, for instance), not as the default. The `client_confirmed` → TAE-verifies handshake it already has is the right shape for that fallback; it just shouldn't be the primary path when full automation is possible.
...
- [x] **Meta description + canonical tag.** Shipped 2026-08-13 for F.R.I. Marketing, live on both `frimarketingllc.com` and `fri-marketing.vercel.app` — verified directly against production, not just committed. Two new `head_tag` fields registered in `contextlayer.site_content_fields` (`seo_meta_description`, `seo_canonical_url`, category `seo`, order_index 20–21) — the first real fields to use the `head_tag` mechanism (migration 317) and the `seo` category (migration 318), both of which had shipped ahead of any real field registration until now. Written via two separate commits on `taemarketing/fri-marketing`'s `main` (`fe277fe`, `68dc575`), matching `update_site_content`'s real one-field-per-call, one-commit-per-call behavior exactly, with matching `contextlayer.site_content_changes` audit rows (`old_value: null`, since neither tag existed before). Also surfaced and fixed a real gap while doing this: the Web Analytics tracking script added earlier the same day had been deployed straight via `vercel deploy --prod` and never committed — repo had silently drifted from what was actually live. Committed and reconciled with a merge before touching anything else.
- [x] **Open Graph + Twitter Card tags** (title/description/image) — controls how a shared link looks everywhere it gets pasted. Shipped 2026-08-13 for F.R.I. Marketing, verified live in production. Three more editable `head_tag`/`seo` fields registered (`og_title`, `og_description`, `og_image`, order_index 22–24) plus a fourth, `page_title` (`selector_type: regex`, order_index 19) — fixing a real bug caught along the way: `<title>` still read "F.R.I. Marketing — The Powerhouse," leftover from the original 3-design-comparison naming, never updated when Powerhouse became the real site. New title: "F.R.I. Marketing | Your Retail Unfair Advantage." Four separate commits, one per field, same pattern as item 4. A new 1200×630 social-share image (`og-image.jpg`, reusing the live site's own hero styling — headless Chrome screenshot of a matching standalone HTML file, not a new design) was committed directly as a static asset, not through the field system — there's still no binary-asset-commit path in `update_site_content`, same known gap noted in the Content-management section above. `og:url`/`og:type`/`og:site_name`/`twitter:card` shipped as static boilerplate, not editable fields — they rarely vary per client (see the commit message for the reasoning). `twitter:title`/`twitter:description`/`twitter:image` deliberately not added: X/Twitter falls back to the equivalent `og:*` tags automatically when absent — confirmed against current documentation before deciding not to build them, not assumed.
- [x] **JSON-LD structured data.** Shipped 2026-08-13 for F.R.I. Marketing, verified live in production — pulled the real `<script type="application/ld+json">` block back off `frimarketingllc.com` and re-parsed it, not just confirmed the commit landed. Used `ProfessionalService` (a schema.org `LocalBusiness` subtype), not generic `LocalBusiness` — more accurate for a B2B retail-representation firm with no walk-in storefront, per current schema.org best practice ("use the most specific accurate subtype"). Covers NAP (name/address/phone — all pulled directly from the live site's own footer, not invented), `foundingDate`, `email`, `areaServed`, and a `contactPoint` block. Deliberately **left out**: `openingHours` (no real hours exist — F.R.I. isn't a walk-in business) and `logo` (F.R.I.'s wordmark is CSS text, not an image file — the OG image is a hero marketing graphic, not a logo, so it's used for `image` only, not `logo`; a real logo schema property would need the still-not-built binary-asset-commit path). One new field, `structured_data_json` (`head_tag`/`seo`, order_index 25) — the whole JSON-LD block as a single field rather than one field per property, since the properties are structurally interdependent and edited together in practice, not independently. Its detection pattern (`<script type="application/ld\+json">([^<]*)</script>`) was verified to round-trip correctly against the real file with a live regex test before committing, not assumed to work.

### GEO — one checklist bullet on an internal task list today, zero code
- [x] **`generate_llms_txt` tool.** Shipped 2026-08-13, live for F.R.I. Marketing — confirmed with a direct fetch against `frimarketingllc.com/llms.txt`, not just a successful commit. Follows the llmstxt.org convention exactly: `# ` title, `> ` blockquote summary (the only structurally-parsed part besides the title), a free-form description paragraph, and a `## Pages` section linking the site's four real sections (home, `#services`, `#why`, `#clients` — all three anchor IDs confirmed to actually exist in `index.html` before writing the links, not assumed). **Deliberately not mechanically generated** — unlike `generate_sitemap_robots`'s crawl, the tool takes summary/description/section links as direct input, the same pattern `save_holiday_page` already established: authored by a person + Claude, committed by the tool. Refactored `generate_sitemap_robots`'s local `commitFile` helper into a shared top-level function both tools now call, rather than duplicating the GitHub Contents API create-or-update logic a second time.
- [x] **Make the AI-crawler policy in robots.txt an explicit, documented choice, not an accident.** Shipped 2026-08-13. `generate_sitemap_robots` gained `ai_crawler_policy`, three presets: `allow_all` (default — the exact behavior every client has always had, so nothing changes unless someone actively picks something else), `block_training_allow_citation` (disallows 7 named training crawlers — GPTBot, ClaudeBot, Google-Extended, Bytespider, Applebot-Extended, Meta-ExternalAgent, CCBot — while leaving 6 citation/answer-engine crawlers allowed — OAI-SearchBot, ChatGPT-User, Claude-SearchBot, Claude-User, PerplexityBot, Perplexity-User), and `block_all_ai` (blocks both groups by name). `get_sitemap_robots_status` gained matching detection (`aiCrawlerPolicy` + `blockedAiBots` in its output) using the identical bot lists, so status reporting can't quietly drift from what generation actually produces. All three presets round-trip tested (generate → detect → matches) before deploy. **F.R.I.'s live robots.txt was deliberately left unchanged** — still `allow_all`, confirmed directly against production after deploy — since choosing to block any of these bots for a real client is a real business decision, not something to make unilaterally while building the capability.
- [x] **Content-structure guidelines, not tooling** — direct-answer-first sections, FAQ-formatted content, question-phrased headings. Written up 2026-08-13 as its own doc, `TAE_GEO_Content_Guidelines.md` — 6 rules plus a worked before/after example using F.R.I.'s own copy. A copywriting pattern for future site builds and copy audits, not something ContextLayer can automate.
...
- [x] **Extend `site_content_fields` with a third `category`: `seo`**, alongside today's `content`/`brand`. Done 2026-08-13 — migration 318 extends the check constraint; purely a grouping label, doesn't affect how a field is read or written (that's `selector_type`). `scan_site_fields`' prefix-based category inference also extended (`seo_` prefix → `seo`, alongside the existing `brand_` → `brand`). No fields registered with it yet — ready for meta description/OG/JSON-LD once those ship. Live in production, `tae-contextlayer` commit `1b63197`.
- [x] **A new write path for `<head>` injection.** Done 2026-08-13 — new `selector_type: 'head_tag'` (migration 317, `contextlayer.site_content_fields` + `html_template` column). Reuses the existing regex-detect-and-splice logic when the tag already exists; when it doesn't, renders `html_template` (a `{{VALUE}}` placeholder) and inserts it before `</head>` instead of erroring — so `update_site_content` now handles first-time setup and later edits through the same call, for both existing values and brand-new tags. Verified with synthetic HTML: insert-when-absent, replace-when-present, duplicate-tag detection, missing-`</head>` failure, and a re-apply idempotency check (second call replaces, doesn't duplicate). Mechanism only — no live field registered for any company yet; that's the next checklist item. Live in production, `tae-contextlayer` commit `4534a99`.
