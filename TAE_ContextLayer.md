...
- ✅ **Holiday theme apply/revert shipped — closes out the "Holiday Site Theming" cadence task that's been sitting Blocked in the Developing Task List since it was first brainstormed.** `contextlayer.holiday_themes` (migration 312) seeds the top 5 **retail-calendar** holidays chosen for what actually matters to F.R.I.'s business as a Walmart/retail broker — Halloween, Black Friday/Holiday Season, Christmas, New Year, Valentine's Day — not generic civic holidays. Each preset is a company-agnostic `field_key → value` override map (accent colors + a themed hero badge), so the same 5 presets work for any future client whose `site_content_fields` define matching keys. New tools: `get_holiday_themes`, `apply_holiday_theme` (snapshots every touched field's current value into `site_content_changes` before overwriting, tagged with a shared `batch_id`), `revert_holiday_theme` (restores each field in the latest unreverted batch to its snapshotted value). **Manual only, both directions** — per the standing rule against scheduled/self-triggered site changes until TAE is out of beta, there is no cron here; apply and revert are both one-click actions a person runs the morning of and the day after.
...
- **What's still explicitly not built:** a real logo image upload (F.R.I.'s wordmark is text, not an image — swapping to a real logo file needs a new binary-asset-commit path, base64 through the same GitHub API, not yet built), font-family swapping, and any UI to edit brand fields or trigger holiday themes beyond the new Holiday Theme control (brand field editing rides the same `CONTENT_MANAGEMENT_UI_ENABLED = false` flag as ordinary content — the backend is ready, the UI is intentionally still parked). The generic-editor convention discussed as a longer-term direction (`data-tae-field="..."` attributes baked into every future TAE-built site, so extraction never needs bespoke per-client regex) was recommended but not applied — F.R.I.'s fields are still class/tag-anchored regex, now just stored as data instead of code.
