...
- ⬜ **Still open: tae-portal's own production deployment doesn't have the ContextLayer env vars yet.** `tae-portal` is live at `portal.theartistevolution.com`, and that deployment's Vercel environment variables still need `CONTEXTLAYER_MCP_URL` (`https://tae-contextlayer.vercel.app/api/mcp`) and `CONTEXTLAYER_BEARER_TOKEN` added — only the local dev env was updated so far. The Console won't work on the real live dashboard until those are set.
- ⬜ **Still open: expose the `contextlayer` schema.** Supabase's Data API only serves schemas on an explicit allowlist (Settings → Integrations → Data API → Settings → Exposed schemas), a project-wide setting shared by all three live apps. `core`/`discovery`/`portal`/`public`/`graphql_public` are live-exposed (confirmed via a direct API call returning `PGRST106`'s hint field, which lists them); `contextlayer` is not, despite the dashboard UI showing it checked and saved — confirmed via a raw REST request bypassing all client libraries that the live PostgREST config genuinely hasn't picked up the change, even after an explicit, confirmed Save. Not a caching delay, not our migrations or grants (independently verified correct) — a stuck/broken save on Supabase's own platform. Next step is a Supabase support ticket with the exact `PGRST106` evidence, since retrying the toggle further hasn't helped. Everything touching `contextlayer` tables (`log_relationship_event`, `add_industry_alias`, `add_brief_entry`, `assign_account_owner`, plus the account-owner/brief sections of `get_client_context` and the event-logging half of `mark_as_client`) stays blocked until this clears — see the July 30 status update above for exactly what still works today regardless.
