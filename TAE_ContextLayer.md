...
- **Industry text is still genuinely free-form and fragments on exact-string grouping** — this one's a real, open finding, not yet addressed. Deliberately left ungrouped rather than fuzzy-matched in SQL — Claude reading the raw grouped list can cluster similar industries far better than a `GROUP BY` can. Two paths if this gets prioritized: a small ContextLayer-side alias table (raw text → canonical category, grown incrementally, doesn't touch Discovery), or a fixed industry taxonomy in Discovery's own capture (bigger — requires an actual taxonomy decision and touches the live interview flow, which stays off-limits absent a specific decision to revisit it).
- ⬜ **Still not started, unchanged:** per-app Postgres-role isolation for the three existing apps (tracked separately in the Supabase audit), the structured "living brief" table (goals/audience/voice/positioning/history — distinct from the engagement/event/mining tools, which track and describe outcomes and volume, not the encoded strategy itself), Discovery's own "never recite known information" hard rule, and connecting GitHub/Basecamp/analytics as further tools.
