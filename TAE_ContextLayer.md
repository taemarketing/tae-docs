...
Answering that requires outcome data that mostly doesn't exist yet — did the client buy what was recommended, did the campaign work, did they renew. Rather than stall there, ContextLayer's job is to notice the gap and act on it, and the right action depends on what kind of gap it is. If it's answerable by connecting a new system — invoicing, a CRM, an ad platform's API — ContextLayer requests that connection. If it's only ever knowable as a human's judgment call — nothing will ever have that sitting in a database — it drafts a request to the person who'd know and sends it through the notification pipeline already built for other purposes today (`portal.notification_queue` / `notification_subscriptions`, live now). Either way, the system tracks its own progress the same way it tracks a client's: as a report, turned inward — not "here's the client's performance," but "here's how much outcome data we actually have, and where the gaps are." That coverage report is what keeps a permanently-partial dataset useful instead of silently stalled.

None of this exists yet beyond the two proven pieces it's built from: the lens framework (discovery-scoped today) and the notification pipeline (used for other things today). Closing the loop — full-lifecycle lenses, gap-triggered connector requests, a self-facing coverage report — is genuine future work. Full detail and the discovery-side half of this lives in `tae-discovery/docs/improvement-loop.md`.
