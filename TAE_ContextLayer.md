...
- ✅ **Fourth tool shipped: `mine_discovery_sessions(group_by, since?, until?, industry?, service?)`** — the descriptive-mining piece. One flexible tool rather than five narrow ones: buckets completed sessions by industry, service, month, year, entry mode, or referral partner, returning count, average MIS scores, average budget signal, urgency breakdown, and conversion rate per group — every average carries its sample size so a thin group never reads as more confident than it is. Verified against the real 15 completed sessions across all six groupings plus filters. Two honest findings surfaced, not fixed here: industry text is genuinely free-form and fragments on exact-string grouping (deliberately left ungrouped rather than fuzzy-matched — Claude reading the raw list can cluster similar industries far better than a `GROUP BY` can); and no session in the current dataset has `partner_id` set, even ones whose transcript clearly involved a referral chain — partner attribution isn't actually being captured today despite the data model supporting it.
