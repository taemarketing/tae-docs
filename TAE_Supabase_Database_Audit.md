...
| `site_analytics_sources` | on, permissive | Added 2026-08-13. Maps a client company to the Vercel project that hosts their site, so `get_site_analytics` knows which Vercel project to query. One row per company for now. First (only, as of this writing) row: F.R.I. Marketing → `prj_c29DfP5wQwqcPTd9CSCHDi4ikGEl`. |
