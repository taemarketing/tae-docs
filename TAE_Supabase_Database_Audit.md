...
*CLI linked and cross-checked against the live database (project `zbwcyjhczzgowmjzgsim`, "TAE Tools", Postgres 17) via `supabase gen types` and `supabase db advisors` — Supabase's own security/performance linter. Everything below is confirmed against what's actually running, not inferred from files. Originally written July 23, 2026; updated July 27, 2026 (tae-portal migration discipline now actually working — see finding 5 update), then again July 27, 2026 (a real cross-repo migration-numbering collision surfaced and got fixed — see finding 5's second update), then again July 31, 2026 (the collision turned out to involve a third repo, `tae-contextlayer`, not two — see finding 5's third and fourth updates and the new `contextlayer` schema section), then again August 1, 2026 (a permanent reserved-number-block fix — finding 5's fifth update), then again August 13, 2026 (a related-but-different `db push` friction the reserved blocks didn't cover — finding 5's sixth update — plus two new `contextlayer` tables, `cl_task_votes` and `site_analytics_sources`), then again August 13, 2026 — same day, a second pass — (the `contextlayer` schema's table inventory had fallen behind the Website-section build: 4 more real tables added to this doc — `site_content_fields`, `site_content_changes`, `holiday_pages`, `site_page_snapshots` — bringing `contextlayer` to 14 tables and the grand total to 43), then again August 13, 2026 — a third pass — (`site_content_fields` gained a third `selector_type`, `head_tag`, plus a new `html_template` column — migration 317, see its own row's update below), then again August 15, 2026 (a Supabase connector became available directly in Claude sessions — confirmed live with a real `list_projects` call returning this exact project — giving `list_tables`/`get_advisors`/`execute_sql` as an in-session alternative to the CLI workflow described above; no findings in this doc changed, just how they can now be re-checked).*
...
**A sixth instance, 2026-08-13 — not a collision (the reserved blocks held, as designed), but a related friction the reserved-block fix didn't eliminate.** `supabase db push` refuses to run at all — not just warns — when the remote ledger has *any* version the local repo's migrations folder doesn't recognize, even if that version belongs to another repo's own reserved block and nothing is actually wrong. This happened twice, in both directions: pushing tae-portal's first `100_tp_...` migration failed because tae-portal's local folder didn't have files for tae-contextlayer's real `022`/`300`–`307`; then pushing tae-contextlayer's next migration failed because *its* local folder didn't have a file for tae-portal's real `100`. Fixed the same way each time — a no-op placeholder file (`select 1;` plus a comment naming the real owner) added to the *other* repo's migrations folder for each version it didn't recognize, never touching the real migration or the ledger's status. **Not a bug in the reserved-block fix** — it solved the actual collision problem it was built for — but worth knowing: each repo's local migration history still needs to stay a superset of every version in the shared ledger, reserved blocks or not, or `db push` simply won't run. The safe habit from finding 5's earlier instances still applies: run `supabase migration list --linked` before assuming a push failure means something is actually wrong.
...
43 tables total across four app-owned groups plus the two intelligence-only schemas already covered above. (`fri_comments` — listed in the original version of this audit — was dropped along with the fri-marketing anon-key fix, finding 6. `client_resources` and `client_files` added 2026-07-27, mirroring `partner_resources`/`partner_files`. The `contextlayer` schema — 14 tables, `tae-contextlayer`'s own — is new to this table since the last update; see its own section directly below. `contextlayer.holiday_themes`, migration 312, is not among the 14 — it was created and dropped the same day, migration 313, superseded by the file-based Holiday Pages design before it ever shipped a second update.)
...
| `cl_task_votes` | on, permissive | Added 2026-08-13. Informal thumbs up/down on "is this cadence task worth building," plus an optional suggested run frequency, both per-vote rather than a single mutable field so opinions stack instead of overwrite. Anonymous by design — `voter_name` is nullable, no uniqueness constraint, internal reference only. |
| `site_analytics_sources` | on, permissive | Added 2026-08-13. Maps a client company to the real infrastructure behind their site — the Vercel project (`vercel_project_id`) that `get_site_analytics`/`get_site_deployments`/`get_dependency_report`/`get_env_vars_redacted`/`get_dns_domain_status` query, and, added same day (migration 309), the GitHub repo (`github_owner`/`github_repo`) that `get_site_content`/`update_site_content` read and commit to. One row per company for now. First (only, as of this writing) row: F.R.I. Marketing → Vercel `prj_c29DfP5wQwqcPTd9CSCHDi4ikGEl`, GitHub `taemarketing/fri-marketing`. |
| `site_content_fields` | on, permissive | Added 2026-08-13 (migration 310). The field map for content/brand/seo editing, moved from hardcoded code to data — one row per company per editable field (`field_key`, `label`, `category`, a stored regex `pattern`, `group_index`). Onboarding a new client's editable fields is now a config insert, not a code deploy. A second `selector_type` mode, `data_attr`, was added additively later the same day (migration 316) alongside the original `regex` mode — see `TAE_Site_Field_Convention.md`. A third mode, `head_tag`, was added the same day again (migration 317, new `html_template` column) for fields that may not exist in the page yet — meta description, canonical link, Open Graph tags, JSON-LD — reusing `pattern`/`group_index` to detect an existing tag, falling back to inserting `html_template` before `</head>` when nothing matches. `category`'s check constraint was extended the same day again (migration 318) from `content`/`brand` to also allow `seo`. Both mechanisms' first real rows landed the same day: `seo_meta_description` and `seo_canonical_url` for F.R.I. Marketing — the first `head_tag` fields and the first `seo`-category fields, live in production on `frimarketingllc.com`. Extended further the same day: `og_title`/`og_description`/`og_image` (three more `head_tag`/`seo` fields), `page_title` (`selector_type: regex`, fixing a stale `<title>` value), and `structured_data_json` (`head_tag`/`seo`, the whole JSON-LD `ProfessionalService` block as one field rather than one field per schema property, since the properties are structurally interdependent) — F.R.I. now has **16 registered fields total**: 7 `content` (6 `regex` + the 1 `data_attr` footer field), 2 `brand` (`regex`), 7 `seo` (6 `head_tag` + 1 `regex` for `page_title`). Counts confirmed directly against the live table, not computed by hand. |
| `site_content_changes` | on, permissive | Added 2026-08-13 (migration 311). Append-only audit log for every content/brand write — old value, new value, `commit_sha`, who, when. Also doubles as the revert mechanism for the now-retired field-override holiday theming (see `holiday_themes`, below). |
| `holiday_pages` | on, permissive | Added 2026-08-13 (migration 313, slot list corrected twice same day by migrations 314–315). Pure metadata for the file-based Holiday Pages system — which of 9 holiday slots exist (New Year's Day, Valentine's Day, Memorial Day, Independence Day, Labor Day, Halloween, Black Friday/Holiday Season, Thanksgiving Day, Christmas Day) and in what order. Whether a slot actually has a designed page is a fact about the repo (`holidays/<key>.html` existing), not a flag in this table. |
| `site_page_snapshots` | on, permissive | Added 2026-08-13 (migration 313). Whole-page HTML snapshot taken immediately before `apply_holiday_page` swaps a holiday page in, so `revert_holiday_page` can restore the exact previous page. Same append-only discipline as `site_content_changes`, just at file granularity instead of per-field. |
...
**Cadence expanded twice more since, 2026-08-04, and each time exposed the same class of bug.** `future` was added first (migration `022_cadence_tasks_future_bucket.sql`, before the reserved-block fix existed — it collided with tae-portal's real `021` and needed the same renumber-and-placeholder treatment one more time), then `daily`/`weekly` (migration `300_tcl_cadence_tasks_daily_weekly_buckets.sql` — the first real use of tae-contextlayer's new `300–399` reserved block, collision-free). Both times, updating the `cl_tasks.cadence` check constraint alone wasn't enough: `tae-contextlayer`'s `add_cadence_task`/`get_cadence_tasks` MCP tools each hard-code their own independent Zod `enum([...])` for the same field, so the tool rejected `daily` at its own input-validation layer on the first attempt, before the request ever reached the database. Both places had to be updated and redeployed. Worth generalizing: any enum-shaped column with a matching MCP tool schema has two sources of truth, not one, and both need to move together.
...
- ✅ **`contextlayer` schema** — created 2026-07-30 when `tae-contextlayer` actually shipped its first tools, exactly on the schedule this recommendation called for ("once ContextLayer is actually being built," not speculatively before). 10 tables now live there — see its own section in the table inventory above.
