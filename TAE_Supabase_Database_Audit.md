...
| `site_content_fields` | on, permissive | Added 2026-08-13 (migration 310). The field map for content/brand editing, moved from hardcoded code to data — one row per company per editable field (`field_key`, `label`, `category` — `content` or `brand` — a stored regex `pattern`, `group_index`). Onboarding a new client's editable fields is now a config insert, not a code deploy. A second `selector_type` mode, `data_attr`, was added additively later the same day (migration 316) alongside the original `regex` mode — see `TAE_Site_Field_Convention.md`. A third mode, `head_tag`, was added the same day again (migration 317, new `html_template` column) for fields that may not exist in the page yet — meta description, canonical link, Open Graph tags, JSON-LD — reusing `pattern`/`group_index` to detect an existing tag, falling back to inserting `html_template` before `</head>` when nothing matches. No rows use `head_tag` yet — the mechanism shipped ahead of any real field registration. |
