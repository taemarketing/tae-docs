...
- **What it does:** The hosted MCP server — the connector between Claude and everything TAE knows about a client. Full strategic brief and complete tool-by-tool build history in `TAE_ContextLayer.md` (this repo) — this entry is a summary, not the full record. As of 2026-08-15, exposes **50 tools** across four groups:
  ...
  - **24 website/site-management tools** — `get_site_analytics`, `get_site_deployments`, `get_dependency_report`, `get_site_uptime`, `get_broken_links`, `get_sitemap_robots_status`, `get_llms_txt_status`, `get_dns_domain_status`, `get_env_vars_redacted`, `get_site_content`/`update_site_content`, `generate_sitemap_robots`, `generate_llms_txt`, `scan_site_fields`, `get_access_requests`/`add_access_request`/`update_access_request_status`, `get_traffic_anomaly`, `get_core_web_vitals`, `get_top_performer`, `get_business_pulse`, `get_underperforming_pages`, and 5 file-based Holiday Pages tools (`list_holiday_pages`, `get_holiday_page`, `save_holiday_page`, `apply_holiday_page`, `revert_holiday_page`). Most read real external infrastructure (Vercel, GitHub, DNS, PageSpeed) rather than generated content; the two `generate_*` tools are the exception — `generate_sitemap_robots` mechanically crawls, `generate_llms_txt` takes authored content as input instead (see `TAE_ContextLayer.md`'s August 13 status update for why those two differ).
  - **13 roster-wide business-operations tools, added 2026-08-15** — no `company_name` needed, each scans the whole active client roster in one call: `get_client_roster_health`, `get_client_activity_digest`, `get_client_appreciation_list`, `get_stale_leads`, `get_dormant_prospects`, `get_lead_scoring`, `get_partner_pipeline` (Client & Sales Relationships bucket), plus `get_weekly_budget_signal_check`, `get_brief_completeness_sweep`, `get_domain_credential_audit` (Finance & Cost / Internal Ops bucket). All built off data that already existed in Supabase with no new tables — see `TAE_ContextLayer.md`'s August 15 status updates for the full per-tool build notes and the honest gaps found along the way (e.g. only 1 of 7 active clients has any brief category filled in).
