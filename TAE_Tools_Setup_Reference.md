...
- **What it does:** The hosted MCP server — the connector between Claude and everything TAE knows about a client. Full strategic brief and complete tool-by-tool build history in `TAE_ContextLayer.md` (this repo) — this entry is a summary, not the full record. As of 2026-08-13, exposes **30 tools** across three groups:
  ...
  - **17 website/site-management tools**, all reading real external infrastructure (Vercel, GitHub, DNS) rather than generated content — `get_site_analytics`, `get_site_deployments`, `get_dependency_report`, `get_site_uptime`, `get_broken_links`, `get_sitemap_robots_status`, `get_dns_domain_status`, `get_env_vars_redacted`, `get_site_content`/`update_site_content`, `generate_sitemap_robots`, `scan_site_fields`, and 5 file-based Holiday Pages tools (`list_holiday_pages`, `get_holiday_page`, `save_holiday_page`, `apply_holiday_page`, `revert_holiday_page`).
